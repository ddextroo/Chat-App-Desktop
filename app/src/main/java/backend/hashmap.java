...
public class hashmap<K, V> {
    ...
    public hashmap(int initialCapacity) {
        table = new entrymap[initialCapacity];
        size = 0;
    }
    ...
    public V put(K key, V value) {
        ...
        if (table[index] == null) {
            table[index] = new entrymap<>(key, value);
        } else {
            entrymap<K, V> entry = table[index];
            ...
            if (entry.getKey().equals(key)) {
                ...
                entry.setNext(new entrymap<>(key, value));
            }
        }
        size++;
        return null;
    ...
    public void resizeBackingTable(int length) {
        ...
        entrymap<K, V>[] entries = new entrymap[length];
        for (entrymap entry : table) {
            if (entry != null) {
                int index =
    ...
    public V get(K key) {
        ...
        entrymap<K, V> entry = table[Math.abs(key.hashCode() % table.length)];
     ...
     */
    public void clear() {
        table = new entrymap[INITIAL_CAPACITY];
        size = 0;
    }
